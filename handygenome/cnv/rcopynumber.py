...
def run_rcopynumber_unified(
    ...
    **kwargs,
    ...
    Returns:
        ...
        merged_gr: Columns ("Chromosome", "Start", "End", "depth_raw", +/- "baf_raw")
    """
    # sanity checks
    if (refver is None) and (cytoband_gr is None):
        raise Exception(f'At least one of "refver" or "cytoband_gr" argument must be set.')
    ...
    if df_written is None:
        # remove unnecessary columns
        depth_df = depth_df.loc[:, required_columns['depth']]
        if baf_df is not None:
            baf_df = baf_df.loc[:, required_columns['baf']]

        # make input data for r-copynumber
        merged_gr, merged_gr_wona = make_merged_gr(depth_df, baf_df, cytoband_gr, refver)
        if compact:
            df_written, coord_converter, rev_coord_converter = make_compact(merged_gr_wona)
        else:
            df_written = make_noncompact_input(merged_gr_wona)
