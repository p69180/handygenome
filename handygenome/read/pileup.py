...
class Pileup(PileupBase):
    ...
    def extend_rightward(self, width, extend_fetchedreads_by=DEFAULT_EXTEND_FETCHEDREADS_BY):
        ...
        if not self.fetchedreads.check_fully_includes(self.chrom, self.end0, self.end0 + width):
            read_addition_width = max(width, extend_fetchedreads_by)
            self.fetchedreads.add_reads(self.chrom, self.end0, self.end0 + read_addition_width)
        # make new pileup
    ...
    def extend_leftward(self, width, extend_fetchedreads_by=DEFAULT_EXTEND_FETCHEDREADS_BY):
        ...
        # extend fetchedreads if needed
        if not self.fetchedreads.check_fully_includes(self.chrom, self.start0 - width, self.start0):
            read_addition_width = max(width, extend_fetchedreads_by)
            self.fetchedreads.add_reads(self.chrom, self.start0 - read_addition_width, self.start0)
        # make new pileup
        new_pileup = self._extend_helper_make_new_pileup(
            chrom=self.chrom,
...
def get_pileup(
    ...
    def supplement_fetchedreads(chrom, start0, end0, bam, fetchedreads):
        if fetchedreads is None:
            fetchedreads = FetchedReads.from_fetch(bam, chrom, start0, end0, readfilter=readhandler.readfilter_pileup)
        else:
