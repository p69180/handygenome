...
def get_refver(url):
    response = common.http_run_urlopen(url)
...
def submain_new(
    ...
    if download_path is None:
        ...
        #common.download(ftp_url, download_path)
        common.download_wget(ftp_url, download_path)
...
def submain_grch37(args, logger):
    asmblspec = libassemblyspec.SPECS["grch37"]
    submain(
...
def submain_grch38(args, logger):
    asmblspec = libassemblyspec.SPECS["grch38"]
    submain(
