...
def unit_job(split_infile_path, split_outfile_path, bam_path_list, id_list, refver, no_matesearch, countonly):
    # basic setup
    ...
    for vr in in_vcf.fetch():
        ...
        update_new_vr(new_vr, fasta, chromdict, bam_dict, no_matesearch, countonly)
...
def update_new_vr(new_vr, fasta, chromdict, bam_dict, no_matesearch, countonly):
...
#                (args.idlist is None) and
#                (args.bamlist_file_path is None)
#                #(args.pon_cohorts is not None)
#            )
#        ):
#            raise Exception(
#                f'Allowed bam file specification arguments '
#                f'(among --bamlist, --idlist, --bamlist-file, --pon-cohorts):\n'
#                f'1) use only --bamlist and --idlist\n'
#                f'2) use only --bamlist-file\n'
#                f'3) use only --pon-cohorts'
#            )
...
def write_jobscripts(
    ...
    for zidx, split_infile_path in common.zenumerate(split_infile_path_list):
        ...
        script_contents = textwrap.dedent(f"""\
            ...
            from handygenome.workflow import get_logger

            #logger = get_logger(
            #    name={repr(__name__.split('.')[-1])},
            #    stderr=False,
            #    filenames=[{repr(logpath)}],
            #)
            ...
            with contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
                try:
                    unit_job(
                        split_infile_path={repr(split_infile_path)},
                        split_outfile_path={repr(split_outfile_path)},
                        bam_path_list={repr(bam_path_list)},
                        id_list={repr(id_list)},
                        refver={repr(refver)},
                        no_matesearch={repr(no_matesearch)},
                        countonly={repr(countonly)},
                    )
                except:
