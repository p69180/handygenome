...
class VcfPlus:
    ...
    def __repr__(self):
        ...
        if self.vplist_filtered is not None:
            result.extend([
                ...
                textwrap.indent(
                    pprint.pformat(
                        self.vplist_filtered[:self._vplist_show_len]), ' '*4)])
            if len(self.vplist_filtered) > self._vplist_show_len:
                result.append(' '*4 + '...')
            result.append(' '*4 
                          + f'{len(self.vplist_filtered)} VariantPlus objects')
    ...
    def write(self, outfile_path, vplist=None, 
              mode_bcftools='z', mode_pysam=None, index=True):
        ...
        with pysam.VariantFile(outfile_path, mode=mode_pysam, 
                               header=self.vcf.header) as out_vcf:
            for vp in vplist:
                if vp.is_sv:
                    out_vcf.write(vp.vr)
...
def get_updated_pysamvcf(vcf, chromdict=None, samples=None, pysamhdr=None, 
