...
gc.sample.stats.modified <- 
  function (seqz.data.all, verbose) {
    if (verbose) {
      message("Collecting GC information ")
    }
    
    n_chr <- table(seqz.data.all$chromosome)
    u_chr <- str_sort(names(n_chr), numeric = TRUE) # from stringr package
    gc1 <- lapply(split(seqz.data.all$depth.normal, seqz.data.all$GC.percent), table)
    gc2 <- lapply(split(seqz.data.all$depth.tumor, seqz.data.all$GC.percent), table)
    res <- rbind(list(unique = u_chr, lines = n_chr, gc_nor = gc1, gc_tum = gc2))
    
    if (verbose) {
      message(" done\n")
    }
    
    return(unfold_gc(res, stats = TRUE))
  }
