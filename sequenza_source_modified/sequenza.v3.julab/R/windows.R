...
windowValues.mod <-  
    function(
        ...
        positions.narm <- na.omit(positions)
        range.pos <- c(min(positions.narm), max(positions.narm))
        if (!is.null(start.coord)) {
            range.pos[1] <- as.integer(start.coord)
        }
        beam.coords <- seq.int(range.pos[1], range.pos[2], by = window.offset)
        ...
        nWindows <- length(beam.coords) - overlap - 1
        pos.cut <- cut(positions, breaks = beam.coords)
        x.split <- split(x, f = pos.cut)
        weight.split <- split(weight, f = pos.cut)
        window.starts <- beam.coords[1:nWindows]
        window.ends <- beam.coords[(1:nWindows) + 1 + overlap]
        idx.list <- lapply(1:nWindows, function(ii) ii + (0:overlap))
        x.window <- lapply(idx.list, function(idx) {
        ...
        result <- list()
        result[[chr]] <- data.frame(
            start = window.starts, 
            end = window.ends,
            mean = window.means, 
            q0 = window.quantiles[1, ],
            q1 = window.quantiles[2, ], 
            N = window.counts
        )
        return(result)
    }
...
windowBf.mod <- 
    function(
        ...
        nWindows <- length(beam.coords) - overlap - 1
        pos.cut <- cut(positions, breaks = beam.coords)
        A.split <- split(Af, f = pos.cut)
        B.split <- split(Bf, f = pos.cut)
        d.split <- split(good.reads, f = pos.cut)
        window.starts <- beam.coords[1:nWindows]
        window.ends <- beam.coords[(1:nWindows) + 1 + overlap]
        idx.list <- lapply(1:nWindows, function(ii) ii + (0:overlap))
        A.window <- lapply(idx.list, function(idx) {
        ...
        result[[chr]] <-
            data.frame(
                ...
                mean = window.quantiles[2, ], 
                q0 = window.quantiles[1, ],
                q1 = window.quantiles[3, ], 
                N = window.counts
            )
